    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="flex justify-between h-16">
            <div class="flex">
                ...
                @role('fundraiser')
                <div class="hidden space-x-8 sm:-my-px sm:ms-10 sm:flex">
                    <x-nav-link :href="route('admin.my-withdrawals')" :active="request()->routeIs('admin.my-withdrawals.index')">
                        {{ __('My Withdrawals') }}
                    </x-nav-link>
                @endrole
                ...
                <x-dropdown align="right" width="48">
                    <x-slot name="trigger">
                        <button class="inline-flex items-center px-3 py-2 border border-transparent text-sm leading-4 font-medium rounded-md text-gray-500 bg-white hover:text-gray-700 focus:outline-none transition ease-in-out duration-150">
                            <div class="flex flex-row items-center gap-x-5"><img class="w-[10px] h-[10px]" src="{{ Storage::url(Auth::user()->avatar)}}"/>{{ Auth::user()->name }}</div>
